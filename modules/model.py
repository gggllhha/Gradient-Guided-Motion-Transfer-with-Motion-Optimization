...
from modules.util import AntiAliasInterpolation2d, TPS, Get_gradient, Get_gradient_nopadding
from torchvision import models
import numpy as np
from modules.util import VGGPerceptualLoss
...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, kp_extractor, bg_predictor, dense_motion_network, inpainting_network, train_params, *kwargs):
        ...
        self.vgg_grad = Vgg19()
        if torch.cuda.is_available():
            self.vgg_grad = self.vgg_grad.cuda()
        self.pyramid_grad = ImagePyramide(self.scales, inpainting_network.num_channels)
        if torch.cuda.is_available():
            self.pyramid_grad = self.pyramid_grad.cuda()
    ...
    def forward(self, x, epoch):
        ...
        # gradient pixel loss  1
        if self.loss_weights['gradient_pixel'] != 0:
            loss = nn.L1Loss()
            value = loss(self.get_grad(generated['prediction']), self.get_grad(x['driving']))
            loss_values['gradient_pixel'] = self.loss_weights['gradient_pixel'] * value

        # branch pixel loss  10
        if self.loss_weights['branch_pixel'] != 0:
            loss = nn.L1Loss()
            value = loss(generated['prediction_grad'], self.get_grad_nopadding(x['driving']))
            loss_values['branch_pixel'] = self.loss_weights['branch_pixel'] * value
