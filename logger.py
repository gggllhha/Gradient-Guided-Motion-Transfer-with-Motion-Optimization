...
class Logger:
    ...
    def load_cpk(checkpoint_path, inpainting_network=None, dense_motion_network=None, kp_detector=None,
                 bg_predictor=None, avd_network=None, optimizer=None, optimizer_bg_predictor=None, optimizer_avd=None):
...
class Visualizer:
    ...
    def visualize(self, driving, source, out):
        ...
        # rec image
        if 'prediction_grad' in out:
            grad_image = out['prediction_grad'].data.cpu().numpy()
            grad_image = np.transpose(grad_image, [0, 2, 3, 1])
        ...
        # attention
        if 'attention' in out:
            attention = out['attention'].reshape(-1, 32, 32, 32, 32)
            attention = attention[:, 0, 0, :, :].data.cpu().unsqueeze(1).repeat(1, 3, 1, 1)
            attention = F.interpolate(attention, size=source.shape[1:3]).numpy()
            attention = np.transpose(attention, [0, 2, 3, 1])
            images.append(attention)
